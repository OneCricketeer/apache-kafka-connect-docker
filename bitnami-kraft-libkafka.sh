...
kafka_validate() {
    ...
    if is_boolean_yes "$KAFKA_ENABLE_KRAFT"; then
        if [[ -n "${KAFKA_CFG_PROCESS_ROLES:-}" ]]; then
            ...
            if [[ "$controller_role_exists" == false ]] && [[ "${KAFKA_CFG_PROCESS_ROLES:-}" != "broker" ]]; then
                warn "KAFKA_CFG_PROCESS_ROLES must include 'controller' for KRaft if not 'broker'"
            fi
        fi

        if [[ -n "${KAFKA_CFG_NODE_ID:-}" ]] || [[ -n "${KAFKA_CFG_CONTROLLER_QUORUM_VOTERS:-}" ]]; then
            if [[ -z "${KAFKA_CFG_NODE_ID:-}" ]] && [[ "$conroller_role_exists" == true ]]; then
            ...
            if [[ "$node_id_matched" == false ]]; then
                if [[ "$controller_role_exists" == true ]]; then
                    print_validation_error "KAFKA_CFG_NODE_ID must match what is set in KAFKA_CFG_CONTROLLER_QUORUM_VOTERS"
                fi
                if [[ "${KAFKA_CFG_PROCESS_ROLES:-}" == "broker" ]]; then
                    warn "KAFKA_CFG_CONTROLLER_QUORUM_VOTERS references an external controller for this broker"
                fi
            fi
        fi
        ...
        if [[ -n "${KAFKA_CFG_LISTENERS:-}" ]]; then
            ...
            IFS=${old_IFS}
            controller_listener_exists=false
            for val in "${listener[@]}";
            do
            if [[ $val == *"CONTROLLER"* ]]; then
                controller_listener_exists=true
                break
            fi
            done
            if [[ "$controller_role_exists" == true ]] && [[ "$controller_listener_exists" == false ]]; then
                warn "KAFKA_CFG_LISTENERS must include a listener for CONTROLLER when KAFKA_CFG_PROCESS_ROLES includes 'controller'"
            fi
        fi
    fi
    ...
    if ! is_boolean_yes "$KAFKA_ENABLE_KRAFT"; then
        ...
        # advertised.listeners config must be empty when process.roles=controller
        if [[ "$controller_role_exists" == true ]] && [[ "$KAFKA_CFG_PROCESS_ROLES" == "controller" ]]; then
            unset KAFKA_ADVERTISED_LISTENERS
            unset KAFKA_CFG_ADVERTISED_LISTENERS
            remove_in_file "$(kafka_get_conf_file)" "advertised.listeners" false
            # sed -i "/^$prop=/d" /opt/bitnami/kafka/config/kraft/server.properties
        fi
    fi
