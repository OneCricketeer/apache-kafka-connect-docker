...
kafka_kraft_storage_initialize() {
    ...
    if grep -Eq "^process.roles=controller$" "$KAFKA_CONF_FILE"; then
        sed -i "s/^advertised\.listeners/#advertised.listeners/g" "$KAFKA_CONF_FILE"
        sed -i "s/^inter\.broker\.listener\.name/#inter.broker.listener.name/g" "$KAFKA_CONF_FILE"
    fi
